...
class PageController extends Controller {
	...
	public function jmap($using, $methodCalls) {
		...
		if (is_array($using) && !is_null($using) && !empty($using) && in_array("urn:ietf:params:jmap:contacts", $using)) {
			if (is_array($methodCalls) && !is_null($methodCalls) && !empty($methodCalls) && !is_null($methodCalls[0][2])) {
				if (strcmp($methodCalls[0][0], "Contact/get") === 0) { //&& isset($methodCalls[0][1]->ids) && is_null($methodCalls[0][1]->ids)) {
					$response = $this->getContacts();
					return new JSONResponse($response);
				} else if (strcmp($methodCalls[0][0], "Contact/set") === 0) {
				...
				return new JSONResponse(array("type" => "urn:ietf:params:jmap:error:notRequest", "status" => 400, "detail" => "Not a valid JMAP request"));	
			}
		} else if (is_array($using) && !is_null($using) && !empty($using) && in_array("urn:ietf:params:jmap:calendars", $using)) {
			if (is_array($methodCalls) && !is_null($methodCalls) && !empty($methodCalls) && !is_null($methodCalls[0][2])) {
				if (strcmp($methodCalls[0][0], "CalendarEvent/get") === 0) { //&& isset($methodCalls[0][1]->ids) && is_null($methodCalls[0][1]->ids)) {
			...
			return new JSONResponse(array("type" => "urn:ietf:params:jmap:error:unknownCapability", "status" => 400, "detail" => "Unknown capability"));
		}
	}

	private function getContacts() {

		// Access all data from the DB and return it in the response (currently running a hardcoded SQL query to read all contacts data from the db)
		...
		foreach ($result->fetchAll() as $contact) {
			...
			// $systemUsersSQLResult is an array of arrays and this is why we iterate with foreach and check in each iteration with in_array()
		...
		*/
		...
		foreach ($result->fetchAll() as $appointment) {
			...
			foreach ($icalobj->tree->child as $node) {
				// Differentiate if the iCal component is an event and only then use it for transformation to JMAP CalendarEvent (i.e. ignore VTODO, etc.)
